...
import com.zerobase.storereservation.exception.exceptiomimpl.AlreadyExistStoreException;
import com.zerobase.storereservation.exception.exceptiomimpl.AlreadyExistUserException;
import com.zerobase.storereservation.repository.PartnerRepository;
...
public class PartnerService implements UserDetailsService{
    ...
    public List<ReservationEntity> getReservations(String storeName, LocalDate date){
    ...
    public ReservationEntity refuseReservation(String storeName, LocalDate date, LocalTime time){
        // 주어진 상점 이름, 예약 날짜, 예약 시간에 해당하는 예약 조회
        Optional<ReservationEntity> reservation = reservationRepository.findByStore_NameAndReservationDateAndReservationTime(
                storeName,
                date,
                time
        );
