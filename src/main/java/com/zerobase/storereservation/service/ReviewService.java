...
import com.zerobase.storereservation.dto.ReviewRequest;
...
public class ReviewService {
    ...
    // 리뷰 생성 메소드
    public ReviewEntity createReview(ReviewRequest createRequest){
        // 넘어온 인자가 유효한지 체크
        validateRequiredField(createRequest, true);

        // 사용자가 존재하는지 체크
        ReserveEntity reserver = this.reserverRepository.findByUserId(createRequest.getReviewerId())
        ...
        // 존재하는 예약 내역인지 확인
        ReservationEntity reservation = this.reservationRepository.findByStore_NameAndReservationDateAndReservationTime(
        ...
        if(!reserver.getPhoneNumber().equals(reservation.getReservationPhoneNumber())){
            throw new RuntimeException("사용자의 정보와 예약자의 정보가 일치하지 않습니다.");
        }

        // 상점의 별점 업데이트
        List<ReviewEntity> storeReviews = this.reviewRepository.findByStoreName(visitedStore.getName());
        double totalRate = storeReviews.stream().mapToDouble(ReviewEntity::getRate).sum();
    ...
    // 리뷰 업데이트 메소드
    public ReviewEntity updateReview(ReviewRequest updateRequest) {
        // 리뷰 조회에 필요한 인자가 다 넘어왔는지 체크
        validateRequiredField(updateRequest, false);

        // 존재하는 리뷰인지 체크
        ReviewEntity review = this.reviewRepository.findByReviewerIdAndStoreNameAndVisitedDateAndVisitedTime(
        ...
        if(updateRequest.getRate() != null) {
            ...
            // 상점의 평점 업데이트
            List<ReviewEntity> storeReviews = this.reviewRepository.findByStoreName(review.getStore().getName());
            double totalRate = storeReviews.stream().mapToDouble(ReviewEntity::getRate).sum();
    ...
    public String deleteReview(ReviewRequest deleteRequest) {
        ...
        // 상점의 평점 업데이트
        List<ReviewEntity> storeReviews = this.reviewRepository.findByStoreName(review.getStore().getName());
        double totalRate = storeReviews.stream().filter(r -> !r.getId().equals(review.getId())).mapToDouble(ReviewEntity::getRate).sum();
    ...
    public String deleteReviewByAdmin(String storeName, String userId, String partnerId, LocalDate date, LocalTime time) {
        ...
        // 상점의 평점 업데이트
        List<ReviewEntity> storeReviews = this.reviewRepository.findByStore(store);
        double totalRate = storeReviews.stream().filter(r -> !r.getId().equals(review.getId())).mapToDouble(ReviewEntity::getRate).sum();
        double averageRate = storeReviews.size() > 1 ? totalRate / (storeReviews.size() - 1) : 0;
        store.setRating(averageRate);
        this.storeRepository.save(store);
